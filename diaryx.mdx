---
title: Diaryx
author: Adam Harris
created: 2025-08-26T20:43:49+00:00
updated: 2025-10-16T18:40:30+06:00
audience:
  - public
  - Digital journal users
  - Markdown users
format: Diaryx v0.8.0
reachable: "[Diaryx Website](https://www.diaryx.org)"
contents:
  - '[What is Diaryx?](/what-is-diaryx.md)'
  - '[Help us bring Diaryx to life](/contribute.md)'
  - '[Read the specification](/diaryx-writing-specification.md)'
  - '[History](/history.md)'
attachments:
  - diaryx-icon.svg
---

<div style="display: flex; align-items: center; gap: 2rem; margin-bottom: 2rem;">
  <h1 style="margin: 0; font-size: clamp(3.5rem, 12vw, 7rem); font-weight: 700; line-height: 1; letter-spacing: -0.03em;">Diaryx</h1>
  <img src="/diaryx-icon.svg" alt="Diaryx icon" style="width: clamp(6rem, 18vw, 10rem); height: auto; flex-shrink: 0;" />
</div>

### Writing connects you to others. And to yourself.

Diaryx is writing software that has unique features, such as audience filtering. It is designed to increase the portability and safety of your writing, and make it easy to share with others.

- [What is Diaryx?](/what-is-diaryx.md)
- [Help us bring Diaryx to life](/contribute.md)
- [Read the specification](/diaryx-writing-specification.md)
- [Use the web app](https://diaryx-org.github.io/diaryx)
